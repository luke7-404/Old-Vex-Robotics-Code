...
//fuction to reset the motor rotation encoder
...
bool latDriveOn = false; // manual toggle for the lateral P loop

void latDrive(double targetDistance){ // delcaires the function and the variable of where we want the bot to be

  double Kp = 0.38; // Kp is a modifier
  double Kd = 0.61;

  double PrevError;

  while((latDriveOn)&&(fabs(targetDistance)>0)){  // 
    ...
    int avgPos = (bakLefPos + bakRigtPos + frntLefPos + frntRigtPos)/4; // gets the average of all four motor positions as a whole number
  
    double error = targetDistance - avgPos; // decleration and updation of the error/ difference term
    double derivative = error - PrevError;

    double latMotorSpeed = error * Kp + derivative * Kd; // decleration and updation of the motor speed

    leftFront.spin(forward, latMotorSpeed, pct); // application of the motorspeed in percent units
  ...
  leftFront.stop(brake); // when the error/ diffence is below 0 it stops the motor
  leftBack.stop(brake);
...
bool rotDriveOn = false; // manual toggle for the turn PD loop

void rotDrive(double targetHeading){ // delcaires the function and the variable of where we want the bot to be

  double turnError = targetHeading - Inertial.heading(); // gets the difference of where bot want to be and where th bot is
  double TprevError; // after every time the function is ran it updates to get the past difference
  double turnDerivative; // gets the difference of the past error and current to predict a future

  double TKp = 0.5; // both Kp and Kd are modifiers
  double TKd = 0.6;

  double rotMotorSpeed; // establisjmnt of motor speed variables 

  while ((rotDriveOn) && (fabs(turnError) > 1)) { // if the error is above or below 0 then it will run the code below
  
    rotMotorSpeed = TKp * turnError + TKd * turnDerivative; // multiplying the variables to get the motor speed

    leftFront.spin(fwd, rotMotorSpeed, pct); // applies the motor speed
    leftBack.spin(fwd, rotMotorSpeed, pct);
    rightFront.spin(reverse, rotMotorSpeed, pct); // flipped motor direction to turn
    rightFront.spin(reverse, rotMotorSpeed, pct);

    turnError = targetHeading - Inertial.heading(); // updating the difference

    vex::task::sleep(20); // the functions cycle, 20 mesc
    TprevError = turnError; // updating the past difference
    turnDerivative = turnError - TprevError; // updating the derivative
  }
  leftFront.stop(brake); // when the error/ diffence is below 0 it stops the motor
  leftBack.stop(brake);
  rightFront.stop(brake);
